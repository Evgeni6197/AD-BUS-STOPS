document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function fetch_opened_order(stop_id, months_to_book , price, order_id){
        ...
        .catch(()=>{
            console.log('catch  in fetch_opened_order  fetch: PUT')
            hide_main_spinner();
            show_message(reload_text);
        })
    }
            ...
            .then(result => {
                ...
                if (!result.db_successful_connections){
                    ...
                    // booking faluire    
                        ...
                        if (!vacant_from){
                            try {
                                document.getElementById('vacant_from_prompt_'+ stop_id).innerHTML = '';
                            }                                
                            catch(error){
                        ...
                        document.getElementById('vacant_from_' + stop_id).innerHTML = vacant_from;                        
                        return_button.click();
                        show_message(try_again_text); 
                    }
                }
