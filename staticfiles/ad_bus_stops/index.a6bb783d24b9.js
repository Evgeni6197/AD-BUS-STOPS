...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    const general_spinner = document.getElementById('general_spinner');
    const mob_narr_spinner = document.getElementById('mob_narr_spinner');
    const dropdown_items = document.querySelectorAll('.dropdown-item');
    dropdown_items.forEach((element)=>{
        element.addEventListener('click',(event)=>{
            general_spinner.classList.remove('transparent');
            if (mob_narr_spinner){
                mob_narr_spinner.classList.remove('transparent');
            }
        });
    });
    ...
    general_index_container.style.opacity = '50%';
